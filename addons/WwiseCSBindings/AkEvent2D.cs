...
[Tool]
public partial class AkEvent2D : Node2D
{

	private new static readonly StringName NativeName = new StringName("AkEvent2D");

	[Obsolete("Wrapper types cannot be constructed with constructors (it only instantiate the underlying AkEvent2D object), please use the Instantiate() method instead.")]
	protected AkEvent2D() { }

	private static CSharpScript _wrapperScriptAsset;

	/// <summary>
	/// Try to cast the script on the supplied <paramref name="godotObject"/> to the <see cref="AkEvent2D"/> wrapper type,
	/// if no script has attached to the type, or the script attached to the type does not inherit the <see cref="AkEvent2D"/> wrapper type,
	/// a new instance of the <see cref="AkEvent2D"/> wrapper script will get attaches to the <paramref name="godotObject"/>.
	/// </summary>
	/// <remarks>The developer should only supply the <paramref name="godotObject"/> that represents the correct underlying GDExtension type.</remarks>
	/// <param name="godotObject">The <paramref name="godotObject"/> that represents the correct underlying GDExtension type.</param>
	/// <returns>The existing or a new instance of the <see cref="AkEvent2D"/> wrapper script attached to the supplied <paramref name="godotObject"/>.</returns>
	public new static AkEvent2D Bind(GodotObject godotObject)
	{
		if (!IsInstanceValid(godotObject))
			return null;

		if (godotObject is AkEvent2D wrapperScriptInstance)
			return wrapperScriptInstance;

#if DEBUG
		var expectedType = typeof(AkEvent2D);
		...
		{
			var scriptPathAttribute = typeof(AkEvent2D).GetCustomAttributes<ScriptPathAttribute>().FirstOrDefault();
		...
		return (AkEvent2D)InstanceFromId(instanceId);
	}

	/// <summary>
	/// Creates an instance of the GDExtension <see cref="AkEvent2D"/> type, and attaches a wrapper script instance to it.
	/// </summary>
	/// <returns>The wrapper instance linked to the underlying GDExtension "AkEvent2D" type.</returns>
	public new static AkEvent2D Instantiate() => Bind(ClassDB.Instantiate(NativeName).As<GodotObject>());
